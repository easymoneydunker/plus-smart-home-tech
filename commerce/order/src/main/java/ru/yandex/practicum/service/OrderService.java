...
public class OrderService {
    ...
    public OrderDto createNewOrder(CreateNewOrderRequest createNewOrderRequest) {
        ...
        Order order = Order.builder().shoppingCartId(shoppingCartId).username(createNewOrderRequest.getUsername())
                .products(createNewOrderRequest.getShoppingCart().getProducts()).state(OrderState.NEW).build();

        Order savedOrder = orderRepository.save(order);

        BookedProductsDto bookedProduct = warehouseClient.assemblyProductForOrderFromShoppingCart(
        ...
        savedOrder.setFragile(bookedProduct.isFragile());
        ...
        DeliveryDto deliveryDto = DeliveryDto.builder().deliveryId(UUID.randomUUID()).orderId(order.getOrderId())
                .fromAddress(fromAddress).toAddress(addressMapper.toAddressDto(order.getToAddress()))
                .state(DeliveryState.CREATED).build();
    ...
    private Order getOrderById(UUID orderId) {
        return orderRepository.findById(orderId).orElseThrow(() -> new NoOrderFoundException("Заказ не найден: " + orderId));
    }
}