...
public class DeliveryService {
    ...
    public void deliverySuccessful(UUID orderId) {
        Delivery delivery = deliveryRepository.findByOrderId(orderId).orElseThrow(() -> new NoOrderFoundException("Не найден заказ: " + orderId));
    ...
    public void deliveryPicked(UUID deliveryId) {
        ...
        ShippedToDeliveryRequest request = ShippedToDeliveryRequest.builder().orderId(delivery.getOrderId()).deliveryId(deliveryId).build();
    ...
    public void deliveryFailed(UUID orderId) {
        ...
        Delivery delivery = deliveryRepository.findByOrderId(orderId).orElseThrow(() -> new NoOrderFoundException("order no found: " + orderId));
    ...
    public BigDecimal deliveryCost(OrderDto orderDto) {
        ...
        Delivery delivery = deliveryRepository.findById(orderDto.getDeliveryId()).orElseThrow(() -> new NoDeliveryFoundException("no delivery found: " + orderDto.getDeliveryId()));
        ...
        BigDecimal addressAddition = warehouseAddress.equals(deliveryStreet) ? BigDecimal.ZERO : step4.multiply(BigDecimal.valueOf(0.2));
